...
qplot(x=supp, y=len, data=ToothGrowth, facets= \~{}dose, xlab="supplements", ylab="Tooth Length", main="Plot show effect of supplement and dosage on ToothGrowth" )+ geom\_boxplot(aes(fill=supp))
```
...
\#\#\#For Dose=0.5
```\{r\}
t\_test\_dose0.5<-t.test(len\~{}supp, paired=FALSE, data=subset(ToothGrowth, dose == 0.5))
t\_test\_dose0.5
```
...
\#\#\#For Dose= 1
```\{r\}
t\_test\_dose1<-t.test(len\~{}supp, paired=FALSE, data=subset(ToothGrowth, dose == 1))
t\_test\_dose1
```
...
\#\#\#For Dose=2  
```\{r\}
t\_test\_dose2<-t.test(len\~{}supp, paired=FALSE, data=subset(ToothGrowth, dose == 2))
t\_test\_dose2
```
...
```\{r\}
t\_test<-t.test(len\~{}supp, paired=FALSE, data=ToothGrowth)
t\_test
```
